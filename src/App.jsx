...
/* ─── Confetti ─── */
const CONF_COLORS = ['#6366f1','#a855f7','#ec4899','#10b981','#f59e0b','#38bdf8'];
    ...
    <header className="site-header">
      <div className="site-header-inner">
        ...
        <div className="site-logo" onClick={()=>setPage('landing')} style={{cursor:'pointer'}}>
          ...
          <span className="logo-text">MaskAI</span>
        ...
        <div className="header-right">
          <button className="header-cta" onClick={()=>setPage('detect')}>
            Try it free <Icon d={I.arrow} size={15}/>
          </button>
  ...
  <div className="landing">
    ...
    <section className="hero-section">
      <div className="hero-left">
        ...
        <h1 className="hero-h1">
          Detect Face Masks<br/>
          <span className="hero-h1-accent">Instantly with AI</span>
        </h1>
        <p className="hero-p">
          Upload any photo and our MobileNetV2 deep learning model tells you
          whether a face mask is present — with confidence scores in under 200ms.
          No data leaves your browser.
        </p>
        <div className="hero-actions">
          <button className="btn-primary" onClick={onDetect}>
            Start Detecting <Icon d={I.arrow} size={16}/>
          </button>
    ...
    <section className="section">
      <div className="section-header">
        ...
        <p className="section-sub">Three steps from photo to prediction</p>
      </div>
      <div className="steps-grid">
        {[
          {n:'01', icon:'📤', title:'Upload', desc:'Drop an image, paste from clipboard (Ctrl+V), or click to browse. Supports PNG, JPG, WEBP.'},
          {n:'02', icon:'🧠', title:'Analyze', desc:'MobileNetV2 processes the image through transfer-learned layers trained on 853 annotated photos.'},
          {n:'03', icon:'📊', title:'Result', desc:'Get instant confidence scores for both classes with a visual breakdown and shareable result.'},
    ...
    <section className="section section-alt">
      <div className="section-header">
        ...
        <h2 className="section-h2">Built for real use</h2>
      </div>
      <div className="features-grid">
        {[
          {icon:'⚡', title:'Sub-second inference', desc:'FastAPI + Uvicorn backend delivers results in under 200ms on average hardware.', color:'#6366f1'},
          {icon:'🔒', title:'Fully private', desc:'Images are processed server-side and never stored. Your data stays yours.', color:'#10b981'},
          {icon:'🎯', title:'81% accuracy', desc:'Validated on a held-out test set of real-world face images with varied lighting.', color:'#a855f7'},
          {icon:'📋', title:'Clipboard paste', desc:'Press Ctrl+V anywhere to instantly analyze an image from your clipboard.', color:'#ec4899'},
          {icon:'📜', title:'Scan history', desc:'Your last 5 scans are kept locally so you can compare results side by side.', color:'#f59e0b'},
          {icon:'📤', title:'Share results', desc:'One-click share or copy your result to send to colleagues or reports.', color:'#38bdf8'},
    ...
    <section className="cta-banner">
      ...
      <div className="cta-banner-inner">
        ...
        <p>Upload your first image in seconds — no account needed.</p>
        <button className="btn-primary btn-lg" onClick={onDetect}>
          Open Detector <Icon d={I.arrow} size={18}/>
        </button>
      </div>
    </section>
    ...
    <footer className="site-footer">
      <div className="footer-inner">
        ...
        <div className="footer-copy">Built with MobileNetV2 · TensorFlow · FastAPI · React</div>
      </div>
    </footer>
  </div>
);
...
const DetectPage = () => {
  ...
  const shareResult = async () => {
    const text = `Face Mask Detection: ${result.label==='with_mask'?'😷 Mask Detected':'🚫 No Mask'} — ${result.confidence}% confidence. Powered by MaskAI`;
    ...
    <div className="detect-page">
      ...
      <div className="detect-left">
        <div className="detect-left-header">
          ...
          <p>Upload an image to analyze mask presence</p>
        </div>
  ...
  return (
    <div className="donut-wrap">
      <svg viewBox="0 0 140 140" className="donut-svg">
        ...
        <text x={cx} y={cy-8} textAnchor="middle" fontSize="16" fontWeight="800" fill="#0f172a" fontFamily="Space Grotesk,sans-serif">853</text>
        <text x={cx} y={cy+10} textAnchor="middle" fontSize="9" fontWeight="600" fill="#94a3b8" fontFamily="Outfit,sans-serif">TOTAL</text>
      </svg>
  ...
  const Chart = ({ title, d1, d2, l1, l2, c1, c2, min, max, yFmt }) => (
    <div className="chart-block">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="chart-svg">
        ...
        {/* x axis labels */}
        {epochs.map((e,i)=>(
          <text key={e} x={toX(i)} y={H-6} textAnchor="middle" fontSize="8" fill="#94a3b8" fontFamily="Outfit,sans-serif">{e}</text>
        ))}
        {/* y axis labels */}
        {[min, (min+max)/2, max].map((v,i)=>(
          <text key={i} x={padL-4} y={toY(v,min,max)+3} textAnchor="end" fontSize="8" fill="#94a3b8" fontFamily="Outfit,sans-serif">{yFmt(v)}</text>
        ))}
        {/* area fills */}
...
const ModelPage = () => (
  <div className="model-page">
    ...
    <div className="model-page-header">
      ...
      <p className="model-page-sub">Deep dive into the MobileNetV2 model powering MaskAI</p>
    </div>
